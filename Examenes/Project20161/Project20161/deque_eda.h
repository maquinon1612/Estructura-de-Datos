...
public:
    ...
    void splice(deque<T>& dos, T pos, T ini, T fin) {

        int i = 1;
        bool encontrado = false, encontradoini = false, encontradofin = false;
        Nodo* posicion = fantasma;
        Nodo* inicio = dos.fantasma;
        Nodo* final = dos.fantasma;

        while (!encontrado && posicion->sig != fantasma) {
            posicion = posicion->sig;
            if (posicion->elem == pos) {
                encontrado = true;
            }
        }
        if (encontrado) {
            while (!encontradofin && final->sig != fantasma) {
                if (!encontradoini) {
                    inicio = inicio->sig;
                    final = inicio;
                    if (inicio->elem == ini) {
        ...
        if (encontradofin) {
            //Quitar de la fila
            final->sig->ant = inicio->ant;
            inicio->ant->sig = final->sig;
            dos.nelems -= i;
            //Añadir a la fila
            posicion->sig->ant = final;
            final->sig = posicion->sig;
            posicion->sig = inicio;
            inicio->ant = posicion;
            nelems += i;
        }
